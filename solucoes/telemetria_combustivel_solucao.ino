...
void loop() {
  ...
  if (percentualInt != ultimoPercentual || consumoInt != ultimoConsumoInt) {
    ...
    // 6. Controle do LED físico de alerta e rótulo de Reserva
    if (volume < 5.0) {
      digitalWrite(pinoAlerta, HIGH); // Acende LED de Alerta
      LCD.setCursor(0, 3);
      LCD.print("!RESERVA!         "); // Rótulo em caso de reserva
      LCD.setCursor(11, 3);
      if (divisaoPorZero) {
        LCD.print("INF");
      } else {
      ...
      digitalWrite(pinoAlerta, LOW); // Apaga LED de Alerta
      LCD.setCursor(0, 3);
      LCD.print("Autonomia:        "); // Restaura rótulo original
      LCD.setCursor(11, 3);
      if (divisaoPorZero) {
        LCD.print("INF");
      } else {
